...
namespace Joker
{
#if DEF_USING_OGL
	template <>
	class JGRA_API CDrawer< eRENDERER_OGL >
		: public CDrawerDC
	{
	public:
		CDrawer();
		...
		virtual void			SetFont					( Joker::CFont & ca2iFont )				{ DoSetFont( ca2iFont );					}
		virtual void			UnsetFont				( Joker::CFont & UNUSED( ca2iFont ) )	{ DoUnsetFont();							}
		virtual COLORREF		SetForegroundColour		( COLORREF crColour )				{ return DoSetForegroundColour( crColour );	}
		virtual COLORREF		SetBackgroundColour		( COLORREF crColour )				{ return DoSetBackgroundColour( crColour );	}
		virtual HPEN			SetPen					( HPEN hPen )						{ return DoSetPen( hPen );					}
		virtual HBRUSH			SetBrush				( HBRUSH hBrush )					{ return DoSetBrush( hBrush );				}
		virtual HBITMAP			SetBitmap				( HBITMAP hBitmap )					{ return DoSetBitmap( hBitmap );			}
	...
	private:
		...
		void			DoDrawBitmap( CRect const & rcDst, IGL2DBitmap * pBitmap, CRect const & rcSrc, BOOL bSrcAlpha=TRUE );
		IGL2DBrush *	DoCreateSolidBrush( COLORREF crColour );
		IGL2DBrush *	DoCreateSolidBrush( HBRUSH hBrush );
	...
	private:
		...
		IGL2DDCRenderTarget *		m_pDcRenderTarget;
		IGL2DBitmapRenderTarget *	m_pBmpRenderTarget;
		IGL2DRenderTarget *			m_pRenderTarget;
		IGL2DBitmap *				m_pOGLBitmap;
	};
#endif
}