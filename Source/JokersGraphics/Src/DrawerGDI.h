...
namespace Joker
{
	...
	public:
		...
		virtual void			AttachImage				( ImagePtr pImage )			{ DoAttachImage( pImage );										}
		virtual	void			DetachImage				()							{ DoDetachImage();												}
		virtual HPEN			SetPen					( HPEN hPen )				{ DoSetPen( hPen );			return SelectGdiObject( hPen );		}
		virtual HBRUSH			SetBrush				( HBRUSH hBrush )			{ DoSetBrush( hBrush );		return SelectGdiObject( hBrush );	}
		virtual HBITMAP			SetBitmap				( HBITMAP hBitmap )			{ DoSetBitmap( hBitmap );	return SelectGdiObject( hBitmap );	}
